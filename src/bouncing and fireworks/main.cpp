...
const float ParticlesPerSecond = 25;
...
void emitParticles(float dt) {
	...
	for(; acc / tpp > 1.f; acc -= tpp) {
		Particle* p = new Particle();
		float r = -5 + 10 * (float)rand()/RAND_MAX;
		p->noBurst = true;
		p->lifespan = 5 + ((float)rand()/RAND_MAX * 1);
		p->pos[0] = 0;
		p->pos[1] = 5 + r;
		p->v[0] = 2 * 5 * (float)rand()/RAND_MAX;
		p->v[1] = -1 + 2 * (float)rand()/RAND_MAX;
